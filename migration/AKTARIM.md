...
**Trimless rehber görselleri (23 dosya, 1 MB):** `dist/shopify-dosyalar/trimless/` klasöründeki `trimless-*.webp` dosyaları aynı adla yüklenir. Firmanın "TRİMLESS LED PROFİLLERİ" klasöründen (2026-09-18) WebP'ye çevrildi, büyütülmedi (`tools/trimless-gorseller.py`). `product.trimless` şablonu bunları kullanır: spot / trimless karşılaştırması, 8 adımlı uygulama, "montajda kapağı çıkarmayın" (doğru / yanlış), 4 adımlı difüzör kapak montajı, 45° köşe kesimi. Firmanın isteğiyle bu beş bölümde ekranda yalnız tek görsellik rehber (`trimless-*-rehber.webp`) görünür; adım, tablo ve metinler arama motorları ve ekran okuyucular için sayfada görünmez metin olarak durur. Bölümdeki "Yalnız özet görseli göster" kutusu kapatılırsa kartlar ve tablo geri gelir. Bölüm metinleri görsellerdeki metinlerin aynısıdır.
